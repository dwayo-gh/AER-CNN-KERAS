...
def build_datasets(train_percentage=0.8, path='Preproc/'):
    ...
    for idx, classname in enumerate(class_names):
        ...
        for idx2, infilename in enumerate(class_files[0:n_load]):
            ...
            # melgram = melgram[:,:,:,0:156]   # just in case files are differnt sizes: clip to 156
            # if melgram.shape[2] < 156:
            #     melgram = melgram[:,:,:,0:melgram.shape[2]]
            if (idx2 < n_train):
                # print(audio_path)
                # print(melgram.shape)
...
if __name__ == '__main__':
    ...
    model.summary()
    # Training the model
    model.fit(X_train,Y_train, validation_data=(X_validation,Y_validation),epochs=30,batch_size=20,verbose=1)
    # Testing the model
